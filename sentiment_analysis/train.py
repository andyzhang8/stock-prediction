...
# Load metadata and prepare categorical features
def load_meta_data():
...
def fetch_data(ticker, sequence_length=SEQUENCE_LENGTH, meta_data=None, ohe=None):
    ...
    # Add metadata features to input data
    ...
    # Safely retrieve last close price as a scalar
...
def predict_trend(model, x_input, scaler, last_close_price):
    with torch.no_grad():
        # Predicted price in scaled form (within the scaled range)
        predicted_price_scaled = model(x_input).item()
        
    inverse_input = np.array([[predicted_price_scaled] + [0] * 7])  # Use [0] * 7 to match 8 feature columns
    predicted_price = scaler.inverse_transform(inverse_input)[0, 0]
    
    # Ensure both prices are compared as floats
    trend = "Bullish" if predicted_price > float(last_close_price) else "Bearish"
